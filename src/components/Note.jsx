        ...
        <div className='shadow-md rounded-md p-4 dark:bg-[#171c25]'>
            ...
            <p className='mt-2 dark:text-[#505057]'>
                {(note?.details?.length >= 400) ? (<span>
                    {note?.details?.slice(0, 400)}...<button className='btn btn-xs bg-green-600 cursor-pointer border-none outline-none'>See More</button>
                </span>) : note?.details}
            </p>
           ...
           {viewNoteData && <ViewNote setViewNoteData={setViewNoteData} viewNoteData={viewNoteData}/>} 
        </div>
    )
}
